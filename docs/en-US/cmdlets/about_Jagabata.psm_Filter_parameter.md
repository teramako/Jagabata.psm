...
### Type "Jagabata.HttpQuery"

This is also an object of `NameValueCollection` type, but it ensures that it is an instance generated by `System.Web.HttpUtility.ParseQueryString(...)`.
And `Jagabata.HttpQuery` object can also be generated from `new Jagabata.QueryBuilder().SomeThingMethods().Build()`.

#### For example:

- `[Jagabata.HttpQuery]::new("field1=foo&field2=bar")`
- `[Jagabata.QueryBuilder]::new().Add("field1", "foo").Add("field2", @("a", "b")).SetOrderBy("id").Build()` => `field1=foo&field2__in=a%2cb`
